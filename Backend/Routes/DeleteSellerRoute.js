...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!isUser.password)
        return res.send(
          "You are sign up from google so please use goolge method for to delete you seller account..."
        );
      const hassPass = await bcrypt.compare(data.pass, isUser.password);
      if (!hassPass)
        return res.send("Invalid password please enter valid password");
      const deletedSellerUser = await becomeInstructor.findOneAndDelete({
